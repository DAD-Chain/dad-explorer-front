 ...
 *  确认插件安装与否: true/false
 */
async function isWalletAvaliable() {
    try {
 ...
 */
async function invokeSmartContract({ operation, args }) {
 ...
 * @param {*} address
 */
async function makePledge({ token, advertiser, bid, orderID }) {
...
async function makeNoPending({ advertiser,adType,token,bid,begin,expire,orderID,capaignName,capaignLink,contries,slots,creative, }) {
...
async function makeCampaignStatusChange({ orderID, status }) {
...
async function makeAppStatusChange({ orderID, status }) {
...
async function makeContribute({userAddress, action, orderID, publiserID}){
    let operation = 'makeContributeTx'
    let args = {
        userAddress,
        action,
        orderID,
...
async function signMessage({ message }) {
    try {
        ...
        console.log(`make plege fail: ${error}`)
        return {
            code: 1,
            result: error.message,
        }
